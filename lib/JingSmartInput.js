...
export default class SmartInput extends Component{
    constructor(props, context) {
        super(props, context);
        this.spanStyle = {backgroundColor: "#f0ad4e", color: "white",  margin:"0 1px", minWidth: 100, height: 30, MozBorderRadius: 2, WebkitBorderRadius: 2, borderRadius: 2, padding: "2px 1px 2px 3px"};
        this.closeStyle ={background: "-webkit-radial-gradient(30% 30%,#fff 5%,#23558f 95%)", margin:"2px 1px 1px 2px", width: 14, height: 14, MozBorderRadius: "50%", WebkitBorderRadius: "50%", borderRadius: "50%", cursor: "pointer", paddingLeft: 3, paddingRight: 4};
        this.divStyle = {verticalAlign: "middle", padding: "1px 3px 3px 2px", position: "absolute", top: 3};
        this.state = {
            inputted: [],
            inputting: ""
        };
    ...
    renderTags(inputted){
        let colseTag = this.handleColse.bind(this);
        const { spanStyle, divStyle, closeStyle } = this;
        let tags = inputted.map((i,j) => <span key={j} style={spanStyle}>{i}<span key={"c_" + j} style={closeStyle} onClick={() => colseTag(i)}>×</span></span>);
        if(tags.length === 0 ){
    ...
    render(){
        ...
        const { lxs, wxs, width, bsSize, style } = this.props;
        return <Input type='text' ref='name' value={this.state.inputting} label={this.props.label}
